...
function MyPage() {
    ...
    const getUserData = async () => {
        setLoading(true);
        try {
            const resp = await dispatch(getUser(userId)).unwrap();
            if (resp) setUserDatas(resp);
        } catch (error) {
            console.log(error);
        } finally {
            setLoading(false);
    ...
    const capitalizedUserName = userDatas
        ? userDatas.name.charAt(0).toUpperCase() + userDatas.name.slice(1)
        : "";
    ...
    const sections = [
        ...
        },
        {
            ...
            path: `/profile/my-results/${userId}`,
